...
from src.Docker.result_plots import plot_docker_results, print_docker_results
...
def main():
    ...
    print_parser = command_parser.add_parser(
        name="print-results",
        description=("Print three tables of all metric values from result CSVs under output."),
    )
    ...
    print_parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to save the plain-text tables (default: <output>/results_tables.txt).",
    )
    print_parser.add_argument(
        "--no-color",
        action="store_true",
        help="Disable ANSI colors in terminal output (useful for logging).",
    )
    ...
    try:
        match args.command:
            ...
            case "print-results":
                logger.info("Printing result tables from %s...", args.output)
                print_docker_results(
                    args.output, output_file=args.output_file, color=not args.no_color
                )
                logger.info("Printed result tables")
            case "stop":
